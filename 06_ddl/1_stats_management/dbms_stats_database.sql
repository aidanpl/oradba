/*
 ...
 Name:          dbms_stats_database.sql

 Purpose:       Refresh/rebuild stats at database level 
 
 Usage:			Key parameter to set each time is 'options' 
 
GATHER: Gathers statistics on all objects in the database.
...
LIST EMPTY: Returns list of objects which currently have no statistics.
 ...
 Example:       sqlplus -s user/pw@sid @dbms_stats_database
 
 Limitations:   Potential for locked stats 

Another issue to consider is whether stats are locked at a table or even database level. By default locked stats will be ignored. See procedures 
		DBMS_STATS.UNLOCK_SCHEMA_STATS(ownname) 
   or   DBMS_STATS.UNLOCK_TABLE_STATS(ownname,tabname) 

   script dbms_stats_unlock_database.sql will unlock at a database level  
...
define script_name = 'dbms_stats_database'
